...
class SongCards extends Component {
  ...
  getScrollState(props) {
    ...
    if ((scrollY - ((ROW_HEIGHT * 4) + (MARGIN_TOP * 2))) > 0) {
    ...
    const itemsToShow = (rowsOnScreen + 4) * ITEMS_PER_ROW;
  ...
  renderSongs(start, end) {
    ...
    for (let i = start; i < end; i += chunk) {
      const songCards = items.slice(i, i + chunk).map((songId, j) => {
        ...
        return (
          <div className="col-1-4 clearfix" key={`${index}-${song.id}`}>
            <SongCard
              dispatch={dispatch}
      ...
      if (songCards.length < chunk) {
        for (let j = 0; j < chunk - songCards.length + 1; j++) {
          songCards.push(<div className="col-1-4" key={`song-placeholder-${(i + j)}`} />);
        }
      }
